...
def robust_center(values: list[float]) -> tuple[float | None, dict[str, Any]]:
    """Centro robusto: mediana con eliminación de outliers por MAD.

    Retorna (center, meta)
    meta incluye n, n_used, outliers_removed.
    """
...
def goal_delta_kcal(goal: GoalType) -> float:
    # MVP: deltas diarios conservadores.
    if goal == "deficit":
        return -400.0
    if goal == "gain":
        return 250.0
...
def target_weekly_weight_change_pct(goal: GoalType) -> float:
    # % del peso corporal por semana (operacional, no clínico)
    if goal == "deficit":
        return -0.5  # -0.5% / semana
    if goal == "gain":
        return 0.25  # +0.25% / semana
    return 0.0
...
def evaluate_weekly_metabolic_profile(
    ...
    *,
    ...
    Entradas:
      profile: {sex, age, height_cm, weight_kg, goal_type, activity_level, daily_target_kcal_override?}
      weights: {current_week: [..], previous_week: [..]}

    Retorna payload listo para UI: targets, adaptación, recomendación, confianza, alertas y preguntas.
    """
    ...
    # Señales: cantidad de puntos, outliers, uso de peso de perfil, faltantes.
    ...
    # --- Adaptation + calorie recommendation update ---
    alpha_prev = _clamp(float(last_alpha or 0.0), 0.0, 0.25)

    # Si override existe, lo respetamos como base para recomendación.
    base_reco = None
    ...
    # Ajuste semanal por discrepancia vs target: búsqueda discreta estable.
    adjustment_kcal = 0.0
    ...
    if decision != "accepted":
        ...
        # Objetivo: acercar delta_obs a delta_target con ajuste conservador.
        # Definimos un score de error (kg) y lo convertimos a kcal/día aproximadas.
        # 1 kg ~ 7700 kcal (aprox operacional)
        if (delta_kg_target is not None) and (delta_kg_obs is not None):
            err_kg = float(delta_kg_obs - delta_kg_target)
            # Si en déficit err es positivo => bajó menos de lo esperado => bajar kcal.
            # Si err es negativo => bajó más de lo esperado => subir kcal.
            # Convertimos a kcal/día sobre 7 días, pero limitamos.
            raw_adj = -float(err_kg) * 7700.0 / 7.0

            # Robustez: si incertidumbre alta, ajustar menos.
            ...
            # Actualizar alpha como parte del modelo: absorbemos parte del error en alpha (suavizado).
            # Alpha sube si el cuerpo "gasta menos" (en déficit, no baja). Disminuye si baja más.
    ...
    if base_reco is not None:
        ...
        # Guardrail: nunca menos de 1200/1400 según sexo, si disponible.
        if sex == "female":
    ...
    payload = {
        ...
        "weights": {
            ...
            "robust_meta": {
                "current_week": cur_meta,
                "previous_week": prev_meta,
            },
        },
        "metabolic": {
...
def main() -> int:
    ...
    raw = args.input_json
    if raw and raw.strip().endswith(".json"):
    ...
    last_alpha = payload.get("last_alpha") if isinstance(payload, dict) else None

    r = evaluate_weekly_metabolic_profile(profile, weights, last_alpha=last_alpha)
