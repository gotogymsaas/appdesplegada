...
def _first(kp: dict[str, dict[str, float]], names: tuple[str, ...]) -> tuple[dict[str, float] | None, str | None]:
    for n in names:
        if n in kp:
            return kp.get(n), n
    return None, None
...
def _scale_from_pose(kp: dict[str, dict[str, float]]) -> tuple[float | None, float]:
...
def _scale_from_side_pose(kp: dict[str, dict[str, float]]) -> tuple[float | None, float]:
...
def _exercise_catalog(locale: str) -> list[dict[str, Any]]:
    # Instrucciones deliberadamente simples y no clínicas.
    # Objetivo: recomendaciones seguras (no “tratamiento”).
    return [
        {
            "id": "chin_tucks",
            "name": "Retracción de mentón (chin tucks)",
            "goal": "control cervical",
            "level": "basic",
            "duration_sec": 90,
            "sets": 2,
            "reps": 10,
            "contra": ["pain_neck", "injury_recent"],
            "cues": ["Alarga la nuca", "No empujes la cabeza hacia abajo", "Movimiento suave"],
        },
        {
            "id": "doorway_pec_stretch",
            "name": "Estiramiento pectoral en marco de puerta",
            "goal": "movilidad pecho/hombro",
            "level": "basic",
            "duration_sec": 120,
            "sets": 2,
            "reps": None,
            "contra": ["injury_recent"],
            "cues": ["Pecho abierto", "Respira lento", "Sin dolor"],
        },
        {
            "id": "scap_retractions",
            "name": "Retracciones escapulares",
            "goal": "estabilidad escapular",
            "level": "basic",
            "duration_sec": 120,
            "sets": 2,
            "reps": 12,
            "contra": ["injury_recent"],
            "cues": ["Hombros lejos de las orejas", "Aprieta entre escápulas", "Control"],
        },
        {
            "id": "wall_angels",
            "name": "Ángeles en la pared (wall angels)",
            "goal": "movilidad torácica/escapular",
            "level": "basic",
            "duration_sec": 120,
            "sets": 2,
            "reps": 8,
            "contra": ["pain_shoulder", "injury_recent"],
            "cues": ["Costillas abajo", "Movimiento lento", "No forzar"],
        },
        {
            "id": "thoracic_extension",
            "name": "Extensión torácica (sobre toalla enrollada)",
            "goal": "movilidad dorsal",
            "level": "basic",
            "duration_sec": 120,
            "sets": 1,
            "reps": None,
            "contra": ["pain_low_back", "injury_recent"],
            "cues": ["Respira profundo", "Evita hiperextender lumbar", "Sin dolor"],
        },
        {
            "id": "dead_bug",
            "name": "Dead bug",
            ...
            "reps": 8,
            "contra": ["pain_low_back", "injury_recent"],
            "cues": ["Espalda neutra", "Exhala al extender", "Control"],
        },
        {
            "id": "bird_dog",
            "name": "Bird dog",
            "goal": "estabilidad lumbopélvica",
            "level": "basic",
            "duration_sec": 180,
            "sets": 2,
            "reps": 8,
            "contra": ["pain_low_back", "injury_recent"],
            "cues": ["Cadera estable", "Largo, no alto", "Respira"],
        },
        {
            "id": "glute_bridge",
            ...
            "reps": 12,
            "contra": ["pain_low_back", "injury_recent"],
            "cues": ["Aprieta glúteos arriba", "Costillas abajo", "No arquees la espalda"],
        },
        {
            "id": "hip_flexor_stretch",
            "name": "Estiramiento de flexores de cadera",
            "goal": "movilidad cadera",
            "level": "basic",
            "duration_sec": 120,
            "sets": 2,
            "reps": None,
            "contra": ["injury_recent"],
            "cues": ["Cadera en retroversión suave", "Glúteo activo", "Sin dolor"],
        },
        {
            "id": "hamstring_stretch",
            "name": "Estiramiento de isquiotibiales",
            "goal": "movilidad posterior",
            "level": "basic",
            "duration_sec": 120,
            "sets": 2,
            "reps": None,
            "contra": ["injury_recent"],
            "cues": ["Espalda larga", "No rebotes", "Respira"],
        },
        {
            "id": "clamshell",
            "name": "Clamshell (abducción cadera en lateral)",
            "goal": "glúteo medio",
            "level": "basic",
            "duration_sec": 180,
            "sets": 2,
            "reps": 12,
            "contra": ["injury_recent"],
            "cues": ["Cadera estable", "Movimiento corto", "Control"],
        },
        {
            "id": "side_steps",
            "name": "Pasos laterales (sin banda o con banda suave)",
            "goal": "control rodilla/cadera",
            "level": "basic",
            "duration_sec": 180,
            "sets": 2,
            "reps": 10,
            "contra": ["injury_recent"],
            "cues": ["Rodillas alineadas", "Cadera atrás", "Pasos cortos"],
        },
    ]
...
def _filter_exercises(catalog: list[dict[str, Any]], user_ctx: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    out = []
    for ex in catalog:
        contra = set(ex.get("contra") or [])
        if contra.intersection(flags):
            continue
        out.append(ex)
    return out


def _pick_by_labels(labels: list[dict[str, Any]], catalog: list[dict[str, Any]]) -> list[str]:
    # Reglas simples y trazables.
    ...
    # Base “para todos”: estabilidad + movilidad suave.
    add("bird_dog")
    return ordered
...
def evaluate_posture(payload: dict[str, Any]) -> PostureResult:
    ...
    locale = str(payload.get("locale") or "es-CO").strip() or "es-CO"
    ...
    # Si no hay escala, no concluimos.
    base_quality = float(_clamp01((float(q_front["score"]) + float(q_side["score"])) / 2.0))
    scale_ok = (scale_f is not None) and (scale_s is not None)
    ...
    if missing:
        ...
        follow_up_questions.append(
            {
                "type": "retake_photos",
                "prompt": "Para analizar tu postura necesito 2 fotos: una frontal y una lateral, con el cuerpo completo (de pies a cabeza) y buena luz.",
                "options": [],
            }
        )
        follow_up_questions.append(
            {
                "type": "safety_pain_check",
                "prompt": "¿Tienes dolor agudo, hormigueo o lesión reciente? Si sí, lo más seguro es pausar y consultar a un profesional.",
                "options": [
                    {"label": "No", "value": "no"},
                    {"label": "Sí", "value": "yes"},
                ],
    ...
    if decision == "accepted" and scale_f is not None:
        ...
        # Head centering: nariz vs punto medio de caderas/hombros
        ...
        if nose and (mid_sh or mid_hip):
            ...
            if head_offset >= 0.14:
                label("head_off_center", severity="mild", confidence=conf, evidence=["front", "head_center_offset"])

        # Knee valgus proxy: ángulo cadera-rodilla-tobillo
        for side_key in ("left", "right"):
            ...
            # En 2D, valgo suele bajar el ángulo (más "cerrado"). Umbral conservador.
            if ang is not None:
    ...
    if decision == "accepted" and scale_s is not None:
        s = float(scale_s)
        ear, _ear_name = _first(kp_s, ("right_ear", "left_ear"))
        sh, _sh_name = _first(kp_s, ("right_shoulder", "left_shoulder"))
        hip, _hip_name = _first(kp_s, ("right_hip", "left_hip"))
        knee, _knee_name = _first(kp_s, ("right_knee", "left_knee"))

        if ear and sh:
            # Normalizamos por la escala del tronco. Usamos distancia horizontal relativa.
        ...
        # Pelvic tilt proxy (muy limitado en 2D): hip vs knee in x relative to trunk scale.
    ...
    # Confianza global
    conf = float(_clamp01(base_quality))
    if decision == "accepted":
        # penalización suave si no detectamos nada (puede ser neutro o insuficiente)
        if not labels and conf >= 0.6:
            conf = float(_clamp01(conf * 0.95))
    else:
        conf = float(_clamp01(conf * 0.65))
    ...
    safety: list[str] = [
        ...
        "Si algún ejercicio genera dolor, detente y ajusta.",
    ]

    catalog = _filter_exercises(_exercise_catalog(locale), user_ctx)
    plan_ids = _pick_by_labels(labels, catalog)
    # materializar ejercicios
    ex_by_id = {e["id"]: e for e in catalog}
    routine = [ex_by_id[i] for i in plan_ids if i in ex_by_id]
    routine = routine[:6]
    ...
    payload_out = {
        ...
        "recommendations": {
            "routine": routine,
            "safety": safety,
            "notes": ["Rutina sugerida 5–10 min/día, 5–6 días/semana (MVP)."],
        },
