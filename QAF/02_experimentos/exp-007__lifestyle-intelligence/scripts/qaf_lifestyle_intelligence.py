...
def _normalize_rhr_inverse(rhr: float | None, *, baseline_7d: float | None) -> tuple[float | None, float, dict[str, Any]]:
    ...
    # comparación relativa: si está +8% o más por encima del baseline -> score baja fuerte
    delta_pct = (r - float(baseline_7d)) / float(baseline_7d)
    # delta 0 -> 0.7, delta +0.12 -> ~0.35
...
def _last_n_days(metrics: list[dict[str, Any]], n: int) -> list[dict[str, Any]]:
    if not isinstance(metrics, list):
        return []
    # ordenar por date asc si existe
    def key(row):
...
def _pick_microhabits(candidates: list[dict[str, Any]], *, memory: dict[str, Any]) -> list[dict[str, Any]]:
    # memory: {"last_ids": ["..."], "recent": {"YYYY-MM-DD": [ids...]}}
    last_ids = []
    try:
    ...
    avoid = set(last_ids[:2])  # evitar repetir inmediatamente
...
def evaluate_lifestyle(payload: dict[str, Any]) -> LifestyleResult:
    ...
    if s_stress_inv is None and stress_sr is not None:
        # stress_sr 0..1 (1=alto). Necesitamos inverso.
        stress_score = _clamp01(1.0 - float(stress_sr))
        stress_conf = c_stress_sr

    # Sueño: si hay self-report calidad, ajusta suavemente.
    sleep_score = s_sleep
    sleep_conf = c_sleep
    ...
    # Hidratación: solo self-report (si viene). No inventar.
    ...
    # DHSS (0..1) — pesos MVP ajustados a datos reales.
    missing: list[str] = []
    ...
    # si no hay hidratación, redistribuir proporcionalmente
    ...
    # score como promedio ponderado por pesos (ignorando confianza para el valor)
    ...
    # confianza global: suma de contribuciones de confianza (ya ponderadas)
    conf = _clamp01(sum(w for _s, w in parts) / max(1e-6, (w_sleep + w_move + w_stress + w_hydr + w_activity)))
    # si faltan señales críticas, colapsar
    ...
    if hydration_score is None:
        # solo sugerir hidratación si el usuario la quiere activar (por ahora: opcional)
        pass
    else:
        if hydration_score < 0.55:
