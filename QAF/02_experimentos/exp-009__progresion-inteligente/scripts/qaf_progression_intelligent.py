...
def _parse_strength_line(text: str) -> dict[str, Any] | None:
    """Acepta: 'sentadilla 3x5x80', 'press banca 4x8 60kg', 'dominadas 4x6'."""
    raw = (text or "").strip()
    if not raw:
        return None
    t = raw.lower()

    # Nombre hasta antes de números
    # match: name + sets x reps (x load optional)
    m = re.search(r"^(?P<name>[^\d]+?)\s+(?P<sets>\d{1,2})\s*[x×]\s*(?P<reps>\d{1,3})(?:\s*[x×]\s*(?P<load>\d+(?:[\.,]\d+)?)(?:\s*kg)?)?\s*$", t)
    if not m:
        # fallback: maybe '3x5x80 sentadilla'
...
def _readiness_score(signals: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    # pesos con redistribución si faltan
...
def _plateau_strength(history: list[dict[str, Any]]) -> tuple[bool, str]:
    # history: last 3 entries (oldest..newest)
    ...
    rpes = []
    for r in rows[-3:]:
        est.append(_safe_float(r.get("est_1rm")))
        rpes.append(_safe_float(r.get("rpe")))
    ...
    # plateau if 2 consecutive improvements < 1.5%
    eps = 0.015
...
def _plateau_cardio(history: list[dict[str, Any]]) -> tuple[bool, str]:
    ...
    if any(v is None for v in mins) or any(v is None for v in rpe):
        return False, "missing_metrics"
    # If minutes stable but RPE rising, plateau/fatigue
...
def evaluate_progression(payload: dict[str, Any]) -> ProgressionResult:
    ...
    if modality == "cardio" and cardio:
        key = "cardio:default"
        plateau, plateau_reason = _plateau_cardio(history.get(key) if isinstance(history.get(key), list) else [])

    action, action_reason = _decision(
        readiness=readiness,
        plateau=plateau,
        rpe=rpe,
    ...
    out = {
        ...
        "micro_goal": micro,
        "meta": {
            "algorithm": "exp-009_progression_intelligent_v0",
...
def render_professional_summary(result: dict[str, Any]) -> str:
    ...
    if result.get('decision') == 'needs_confirmation':
        lines.append("Para ajustar tu progresión con precisión, necesito 2 datos rápidos.")
    ...
    mg = result.get("micro_goal")
    if mg:
        lines.append(f"Micro-objetivo: {mg}")
    if conf.get("missing"):
        lines.append(f"Faltan: {', '.join([str(x) for x in conf.get('missing')])}")
    return "\n".join(lines).strip()
