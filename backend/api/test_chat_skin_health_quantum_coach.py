...
class ChatSkinHealthQuantumCoachTests(TestCase):
    ...
    @patch("api.views.requests.post")
    @override_settings(SECURE_SSL_REDIRECT=False)
    def test_skin_analysis_does_not_fallback_to_internal_summary_when_n8n_empty(
        self,
        mock_post,
        mock_get,
        mock_eval,
        ...
        mock_render.return_value = "RESUMEN TÉCNICO SKIN"
        mock_post.return_value = Mock(status_code=200, json=lambda: {"output": ""})

        resp = self.client.post(
            "/api/chat/",
            {
                "message": "Vitalidad de la Piel",
                "sessionId": "skin-coach-2",
                "attachment": "https://test.blob.core.windows.net/attachments/foto.jpg?sig=1&se=1&sp=r",
                "attachment_text": "",
                "username": "juan",
            },
        ...
        self.assertNotIn("RESUMEN TÉCNICO SKIN", out)
        self.assertIn("Quantum Coach", out)
