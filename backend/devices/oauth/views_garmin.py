...
def garmin_authorize(request):
    ...
    redirect_uri = settings.GARMIN.get("REDIRECT_URI")
    scope = settings.GARMIN.get("SCOPE")
    auth_url = settings.GARMIN.get("AUTH_URL")

    if not client_id or not redirect_uri or not auth_url:
        return Response({"detail": "Configuración Garmin incompleta"}, status=500)

    state = signing.dumps({"uid": user.id}, salt="garmin-oauth")
...
def _get_uid_from_state(state: str) -> int:
    try:
        data = signing.loads(state, salt="garmin-oauth", max_age=900)
        return int(data["uid"])
    except Exception:
        if state.isdigit():
            return int(state)
...
def garmin_callback(request):
    ...
    if not client_id or not client_secret or not token_url:
        return Response({"ok": False, "error": "Configuración Garmin incompleta"}, status=500)
    ...
    if r.status_code != 200:
        return Response({"ok": False, "garmin": data}, status=400)
    ...
    return redirect(f"{frontend_url}?oauth=success&provider=garmin")
